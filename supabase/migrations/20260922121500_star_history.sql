-- A Star can end without deleting the Memory or Collection relationships it created.
alter table public.user_stars
  add column unstarred_at timestamptz;

create index user_stars_active_user_idx
  on public.user_stars (user_id, starred_at desc)
  where unstarred_at is null;

create function public.apply_github_star_snapshot(
  p_user_id uuid,
  p_rows jsonb,
