...
- **2026-09-22 · Ask 历史会话与斜杠命令等宽对齐、材质重塑与排版减负**：按用户反馈系统性解决 Ask 浮动舱的多项体验断点：
  1. **等宽对齐**：明确心智边界，输入胶囊、斜杠命令面板（`AskSlashMenu`）与历史会话列表（`AskHistoryView`）同属索引与选择层，统一定格在紧凑等宽 `max-w-xl xl:max-w-2xl`（576px ~ 672px），消除从斜杠命令面板进入历史列表突然拉宽至 768px 的视觉跳动；仅在会话展开时（`dockView === 'chat' && isExpanded`）才舒展拓宽至 `max-w-2xl xl:max-w-3xl`；
  2. **Apple Liquid Glass 材质净化**：彻底清除历史条目与命令面板中的外部弥散黑色硬阴影与实体上浮，统一使用顶部 1px 晶莹高光（`shadow-[inset_0_1px_...]`）与石墨发丝微暗描边（`border-black/[0.08]`）；隐藏全舱原生灰色滚动条；
  3. **命令菜单与卡片交互升级**：命令菜单去过白、去刺眼实心蓝块，改为清透半透明胶囊与整行冰蓝微光（`bg-primary/[0.07]`）；推荐卡片增强星标蓝微光 hover 反馈；Agent 气泡强化浅色对比度与冷玉高光；
  4. **历史会话视窗一体化与排版减负**：历史视窗重构为完整一体化舱体（顶栏柔和内嵌微分割）；条目采用无多重边框的整行交互；标题首字强制左平齐，当前会话标签尾置精致微胶囊；彻底删除时钟小图标、绿灯闪烁、单轮（`1 轮`）冗余噪音；副行极简化为「相对时间 · 推荐数量 · 轮数」，信息密度大幅呼吸透气。
  契约同步、单测全绿（54 套件 294 项）。见 `logs/2026-09-22-ask-history-and-slash-compact-width.md`。

- **2026-09-22 · Ask 模块 Apple Liquid Glass 物理质感升级与命令/会话互斥聚焦**：根据用户对“泛白牛奶雾缺少 Apple 质感”及“对话与斜杠菜单重合可读性差”的反馈，系统性重构 Ask 模块的流体毛玻璃与交互体系。将底层氛围层升级为 Apple 级多层物理流体结构（底色高透光白玉/石墨渐变、微晶磨砂 SVG 噪点层消除塑料色带、电光蓝/水青双径向色散流光，以及 S 型非线性光学羽化遮罩），复合 `backdrop-blur-3xl` + `backdrop-saturate-[220%]`，兼顾了底层仓库卡片的晶莹散射与对话区域的高清晰度；落地**命令与会话流互斥聚焦（礼貌避让）交互机制**：键入 `/` 时上方对话流临时平滑收拢隐去，纯净展现 `/history` 与 `/new` 选项卡，按 Backspace 或 Esc 退出时对话流无缝弹回恢复，彻底消除双层文字重叠碰撞。全库 54 套件 294 项单测、TypeScript 与 Biome 门禁全绿。见 `logs/2026-09-22-ask-apple-liquid-glass-refinement.md`。
...
- **2026-09-23 · 记忆唤醒卡片标题与收藏时间垂直对齐修复**：针对记忆唤醒卡片（`ResurfaceCard`）首行中左侧仓库名与右侧相对收藏时间文字基线错位问题，定位根因（原父级使用 `items-start` 导致 14px `text-body` 与 12px `text-caption` 顶边对齐，产生 3px 文字底边悬空落差）。将容器调整为 `items-center` 并为右侧时间补充 `leading-5`（20px），使左右两端在同一盒模型内中轴对齐，文字基线落差归零至亚像素级，完美适配已归档徽标与超长截断场景。四道门禁与 294 项测试全绿。
