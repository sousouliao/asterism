...
### 2. 同步 starred（完整对账与历史）

- [x] 首次登录后可全量拉取用户的 starred 仓库。
- [x] 每次完整拉取当前 Star 列表，再原子对账新增、取消与重新 Star；只在全部分页成功后应用快照。
- [x] 取消 Star 的仓库进入历史，保留 Memory 与 Collection；重新 Star 复用原记录。
- [x] 首次连接后保存加密的 GitHub 凭据，开站时静默补同步，定时任务在关站后继续同步；凭据失效时提示重新连接。
