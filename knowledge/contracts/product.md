...
- **Ask Asterism 私有问答（GitHub #41，已授权）**：顶栏入口 / 全局快捷键唤出的 Command Palette 式问答弹层。基于统一 Retrieval 在个人库内完成候选召回（top-K + Match Explanation），再由用户 BYOK 的 OpenAI 兼容 LLM 生成综合回答（ADR 0042）；回答的推荐列表与引用标签经客户端校验（候选索引 ∈ 召回集）后由本地数据渲染，点击直达 Repo Quick Look；库内无匹配时不调用 LLM，明确告知未找到。支持快捷追问（每轮仍基于本地检索重新召回）；双语、键盘友好（Esc 关闭、Tab 导航）与移动端自适应。未配置 key 时引导配置，无抽取式兜底。
