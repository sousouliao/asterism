...
- **Ask 生成（ADR 0042 / 0044）**：Ask Asterism 的回答生成使用用户自带 key 的 OpenAI 兼容 LLM（BYOK，v1 固定 Provider 枚举：DeepSeek / OpenAI / Groq / OpenRouter）；key 仅存浏览器本地，经无状态 Edge Function `ask-generate` 透传，服务端不保存 credential。检索与证据严格限于个人库，不联网搜索；`whySaved` / `note` 原文发送给所选 Provider 前必须显式同意。回答以流式 Markdown 正文呈现，末尾推荐索引经客户端校验后由本地数据渲染为证据卡片；未配置 key 时提供配置引导，不提供无 LLM 的兜底回答。Settings 提供还原的连接管理体验（ADR 0043）：具名连接的新建 / 编辑 / 启停 / 删除、模型检测（失败可手填模型 ID）、连接探针与活跃连接偏好；连接与 key 存浏览器本地库，激活连接即写入 Ask 配置且必须先通过出网同意，「包含笔记」偏好可关闭 Memory 原文进入 prompt。
...
- **Ask Asterism 私有问答（GitHub #41，已授权）**：常驻页面底部的直接输入区；全局快捷键（⌘K / Ctrl K）聚焦 composer，而不是唤起弹层。基于统一 Retrieval 在个人库内完成候选召回（top-K + Match Explanation），再由用户 BYOK 的 OpenAI 兼容 LLM 流式生成综合回答（ADR 0042 / 0044）；正文为受约束的 Markdown，推荐列表与引用标签经客户端校验（候选索引 ∈ 召回集）后由本地数据渲染，点击直达 Repo Quick Look；库内无匹配时不调用 LLM，明确告知未找到。支持快捷追问（每轮仍基于本地检索重新召回）与停止生成；双语、键盘友好与移动端自适应。未配置 key 时引导配置，无抽取式兜底。
