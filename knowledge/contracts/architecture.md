...
> Ask 生成走「目录常驻 Agent + 客户端 BYOK」边界（ADR 0042 / 0044 / 0045）：Web 在浏览器内组装分档目录（完整行 / 精简行 / 分组计数）作为稳定 system 前缀，Agent 循环通过本地确定性工具 `filter` / `search` / `expand` 取证；循环与数据都在客户端，Ask 不再读取 `user_repo_embeddings`。consent v3 通过后才调用无状态 Edge Function `ask-generate`。该函数只校验 Supabase JWT、按 Provider 枚举（DeepSeek / OpenAI / Groq / OpenRouter）解析固定上游 URL、透传客户端持有的 `Authorization`、`tools` / `tool_choice` 与 `tool` 角色消息；生成动作以 `stream: true` 转发上游并把 OpenAI 兼容 SSE 转换为 Asterism 协议（`event: delta | tool_call | error | done`），响应头超时 60 秒、流空闲超时 30 秒。单条消息上限 250_000 字符、合计 600_000，服务函数自身的 CPU/内存健康，不是成本控制。同一函数另承载 `models`（`GET /models` 模型检测，去重排序封顶）与 `test`（连通性 + 工具调用 + 长上下文定位）动作，二者仍为 JSON（ADR 0043 / 0045）；不存储、不日志化任何 credential 或请求体，不接受白名单之外的 base URL（消除 SSRF 面）。回答正文为流式 Markdown；推荐 `repoId` 必须属于本轮已 `expand` 的集合，由本地数据渲染。空库不调用上游。预算耗尽与未找到是两个终止状态。探针未同时证明工具调用与长上下文时，客户端降级到 ADR 0042 的固定 top-K 流程。除 consent v3 路径外，Memory 原文仍不离开浏览器；笔记只在 `expand` 且用户打开「包含笔记」时按需送出。Settings 的连接管理（具名连接、模型检测、能力探针、活跃连接与笔记偏好，ADR 0043）运行在浏览器本地连接库上（`asterism:ai-connections / ai-settings` 版本化键），key 明文只存本浏览器，与服务端凭据架构无关。
