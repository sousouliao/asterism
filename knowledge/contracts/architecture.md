...
> Ask 生成走「本地检索 + 客户端 BYOK」边界（ADR 0042 / 0044）：Web 在浏览器内用统一 Retrieval 召回个人库 top-K 候选，把候选结构化数据（含 `whySaved` / `note` 原文）与问题组装为 prompt，经用户显式同意后调用无状态 Edge Function `ask-generate`。该函数只校验 Supabase JWT、按 Provider 枚举（DeepSeek / OpenAI / Groq / OpenRouter）解析固定上游 URL、透传客户端持有的 `Authorization`；生成动作以 `stream: true` 转发上游并把 OpenAI 兼容 SSE 转换为 Asterism 协议（`event: delta | error | done`），响应头超时 60 秒、流空闲超时 30 秒。同一函数另承载 `models`（`GET /models` 模型检测，去重排序封顶）与 `test`（最小生成探针）动作，二者仍为 JSON（ADR 0043）；不存储、不日志化任何 credential 或请求体，不接受白名单之外的 base URL（消除 SSRF 面）。回答正文为流式 Markdown；推荐与引用必须通过客户端校验（候选索引 ∈ 召回集合），由本地数据渲染；召回为空时不调用上游，直接返回未找到。除该显式同意的路径外，Memory 原文仍不离开浏览器。Settings 的连接管理（具名连接、模型检测、探活、活跃连接与笔记偏好，ADR 0043）运行在浏览器本地连接库上（`asterism:ai-connections / ai-settings` 版本化键），key 明文只存本浏览器，与服务端凭据架构无关。
