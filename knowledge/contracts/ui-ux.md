...
- **分层响应式宽度体系**：
  - **未激活态（Idle 胶囊输入条）**：`max-w-xl xl:max-w-2xl`（576px ~ 672px），如同 Spotlight 般轻巧聚焦，避免未提问时占位符横向空旷冗长，提升主界面通透感。
  - **对话展开态（Thread Active 气泡流）**：`max-w-2xl xl:max-w-3xl`（672px ~ 768px），展开后平滑拓宽，保障 Markdown 正文达到单行 65~80 字符的黄金阅读行宽，并使推荐卡片内的语言色点、仓库全名、检索匹配 Badge 标签簇与星标数横向舒展排布，杜绝拥挤折行。
- **气泡交错节奏与材质**：
  - **用户提问气泡**：贴右排布，`ml-auto max-w-[80%] xl:max-w-[75%]`，采用深海石墨冷晶材质（微冷沉静渐变 + 水晶内高光），自适应字数宽度，为左侧留出充裕负空间。
  - **Asterism 助手气泡**：贴左排布，`mr-auto w-full max-w-[95%] sm:max-w-[92%]`，采用凝脂冰霜白玉微冷材质（顶部入射高光 + 锐利字体对比度），右侧刻意保留 8% 呼吸空隙，形成生动的双向对话张力。
  - **无外部硬阴影**：气泡整体采用通透内高光折射（`shadow-[inset_0_1px_...]`），禁用浑浊的外部黑色大阴影。
