...
- **Liquid Glass 氛围流体层**：有会话展开时，自底部向上延伸最高 `min(52rem, 92vh)` 的流体氛围层，采用 `backdrop-blur-2xl` + 底部微弱冷光晕染（`radial-gradient`）及自下而上线性羽化遮罩（`mask-image`），自然消解与底层页面的生硬交界，杜绝“全包裹矩形大硬框”。
- **分层响应式宽度体系**：
