...
GitHub 会话中的 provider token 缺失时，先检查受信存储的连接。仅当两者都不可用或存储连接已失效时才提示重新连接。连接问题只影响同步能力，不属于全局应用故障。不得使用横跨 App Shell、推挤页面内容的持久 banner。

- Topbar：保持清爽无感，不常驻显式同步或重连按钮；日常同步由开站静默同步与后台定时任务驱动。
- User Menu：承载手动同步与会话恢复入口。连接正常时提供「同步」项；连接失效时以简短标题和说明解释 GitHub 连接已过期，并提供 warning 风格的「重新连接 GitHub」入口。技术性的 provider token / authorization 描述不得暴露给普通用户。
- Browse / Dashboard 空状态：完全无仓库时的主操作在需要恢复时直接显示「重新连接 GitHub」，已连接时显示「同步 Star」。
- Toast 只报告恢复或同步失败，不承担持久状态提示；恢复状态不得通过 modal 阻断浏览已有仓库。
...
使用底部居中 dock 变体——`max-w-2xl`（672px）、贴底（`bottom-4` / `sm:bottom-6`）。未提问时仅渲染独立的 `rounded-full` 输入胶囊（pill composer，48px 高度，无多余外框嵌套与空态提示大白框），绝不遮挡主内容区；提问后对话气泡直接在输入框正上方生长悬浮展示（无死板外层大卡片或硬标题框包裹，`max-h-[min(32rem, calc(100dvh - 8rem))]`，超出后透明通道内部滚动并保持贴底），右上角提供轻量悬浮关闭胶囊，并支持 Esc 快捷收起回到纯输入框状态。
外层全宽包装必须是 `pointer-events-none`，只有气泡与输入框 `pointer-events-auto`，避免挡住两侧页面点击。
Ask dock 是浮在页面上的 overlay，不得用布局占位把主内容顶上去。
消息布局：双侧气泡模式。用户消息为石墨蓝调微渐变流体玻璃气泡靠右（尾角收窄指向 composer），Asterism 回答为晶莹通透的 Liquid Glass 气泡靠左（带星光微身份标示、流式 Markdown 正文与内嵌微毛玻璃证据卡片）；气泡与底部输入框整体采用高阶透明渐变模糊（backdrop-blur-2xl + backdrop-saturate-[190%] + 顶部高光反射亮线），气泡直接悬浮于网页底层内容之上，呈现通透灵动的 Liquid Glass 视觉质感。新消息
...
### Ask Dock Pattern · 问答对话舱模式

Ask 对话舱是悬浮于主界面底部的智能问答与语义检索交互中枢。其布局、材质与阅读节奏遵循如下设计准则：

- **主工作区居中对齐**：桌面端（`≥1024px` 即 `lg` 断点）存在 `w-60`（240px）左侧固定边栏。AskDock 根节点与氛围背景层采用 `fixed bottom-0 right-0 left-0 lg:left-60` 约束基准，使得内部元素精准锁定在右侧主内容区（`calc(100vw - 15rem)`）几何中心，与星标卡片网格与顶栏完全对齐；同时保护左侧导航栏独立纯净，不受底部向上渐隐遮罩污染。窄屏（`<1024px`）无边栏，自动退化为全视口满宽居中。
- **Liquid Glass 氛围流体层与斜杠命令互斥聚焦**：有会话展开或唤起 `/` 斜杠菜单时，自底部向上延伸最高 `min(52rem, 92vh)` 的流体氛围层，采用 Apple 级物理通透底色（浅色 `from-white/80 via-white/45 to-transparent`，暗色 `from-[#0B0E13]/90 via-[#0B0E13]/55`，结合微晶磨砂粒子层与双径向电光蓝/水青流光，杜绝泛白牛奶雾遮蔽）+ `backdrop-blur-3xl` + `backdrop-saturate-[220%]` + `backdrop-brightness-[103%]`，自然消解与底层页面的生硬交界。配套的斜杠命令浮层（Slash Menu）与历史卡片流（Memory Tiles）统一具备 1px 镜面发丝倒角描边（`border border-white/80 dark:border-white/15`）与双层 Inset 物理高光。**会话流与命令互斥聚焦机制**：当在输入框键入 `/` 触发命令菜单时，上方对话气泡流临时平滑收拢避让，消除双层浮层重叠与穿透冲突；当删除 `/` 或按 Esc 退出时，对话流无缝平滑恢复展开，保障最佳单焦点沉浸感。
- **分层响应式宽度体系**：
  - **未激活态与索引选择态（Idle 胶囊输入条 / Slash 命令菜单 / 历史会话列表）**：统一保持紧凑等宽 `max-w-xl xl:max-w-2xl`（576px ~ 672px），如同 Spotlight / Command Palette 般轻巧聚焦。键入 `/` 呼出命令面板、回车进入 `/history` 历史会话列表、按 Esc 返回退出全程严丝合缝等宽，杜绝不必要的横向伸缩跳动（Width Jitter），保持主界面通透与输入框尺寸稳定。
  - **对话展开态（Chat Thread Active 气泡流）**：仅在真正载入或提问展开具体问答会话（`dockView === 'chat'`）时平滑拓宽至 `max-w-2xl xl:max-w-3xl`（672px ~ 768px），保障 Markdown 正文达到单行 65~80 字符的黄金阅读行宽，并使推荐卡片内的语言色点、仓库全名、检索匹配 Badge 标签簇与星标数横向舒展排布，杜绝拥挤折行。
- **气泡交错节奏与材质**：
  - **用户提问气泡**：贴右排布，`ml-auto max-w-[80%] xl:max-w-[75%]`，采用深海石墨冷晶材质（微冷沉静渐变 + 水晶内高光），自适应字数宽度，为左侧留出充裕负空间。
  - **Asterism 助手气泡**：贴左排布，`mr-auto w-full max-w-[95%] sm:max-w-[92%]`，采用凝脂白玉微冷材质。浅色模式使用 `border border-black/[0.08]` 石墨发丝微暗描边收拢轮廓，彻底杜绝白底融化看不清边框的问题；顶部叠加 1px 晶莹入射反光 `shadow-[inset_0_1px_1.5px_rgba(255,255,255,1)]`，深色使用 `dark:border-white/[0.14]`，与底部输入胶囊保持 100% 材质血统一致。气泡内推荐卡片使用 `border border-black/[0.07] bg-white/70` 建立清晰微反差层级。
  - **无外部硬阴影与无缝隐形滚动**：气泡整体采用通透内高光折射（`shadow-[inset_0_1px_...]`），禁用浑浊的外部黑色大阴影；对话舱与历史列表内部滚动层统一隐藏原生滚动条（`[scrollbar-width:none] [&::-webkit-scrollbar]:hidden`），移除强制预留槽位，保留平滑的手势与滑轮自然滚动，保障浮动毛玻璃纯净无杂质。
  - **纵向阅读视野**：滚动区最大高度为 `max-h-[min(36rem, calc(100dvh - 8.5rem))]`，多出 4rem 沉浸阅读空间并保有顶部安全透气边距。
- **无感收放与水平 { 形渐变流光拱（Fluid Luminous Bracket Crest）**：
  - **移除独立关闭按钮**：彻底废除右上角违和的硬胶囊关闭按钮，转为符合人机直觉的“抽屉式无感收放”；
  - **Click Outside & Esc 收起**：展开状态下点击外部空白处或按 Esc（输入框无内容时），对话流平滑收拢，保留对话上下文不丢失；在收起状态下再次按 Esc 可彻底重置清空；
  - **水平 { 形渐变流光拱**：收起状态下在输入框上方横跨展开一条形如水平横卧大括号 `{` 的高阶渐变流光拱（`AskLuminousBracket`）；双曲波浪两翼平滑贴合输入框上沿左右淡出，中央汇聚为优美的向上尖角与高斯模糊双层电光蓝微光光晕（`filter: blur` + `#38bdf8` ~ `#818cf8` 渐变）与白金高光脊线；悬停时微幅上浮与流光提亮，点击流光任意区域、输入框重新聚焦或发送新问题时自动平滑恢复展开。展开状态下提供微弱向下折叠拉手（`ChevronDownIcon`）。
