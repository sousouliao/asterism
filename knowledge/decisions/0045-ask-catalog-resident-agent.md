...
- Status: Accepted（第 5 / 6 条已被 ADR 0046 撤销）
- Date: 2026-09-21
- Amended by: ADR 0046（取消能力分级降级与同意重签）
...
5. ~~**能力分级。**~~ 已由 ADR 0046 撤销：静默降级把可用模型挡在门外，
   且被降级到的固定流程本身更差。Agent 成为唯一路径。

6. **consent v3。** 出网范围从「至多 12 条候选」变为「全库元数据 + 按需笔记」，
   同意对话如实披露。~~v1 / v2 同意不得静默升级~~ 已由 ADR 0046 撤销：
   旧同意向前迁移，不再要求重签。笔记仍受「包含笔记」偏好约束。
...
- ~~弱模型走固定流程~~：见 ADR 0046，不调工具的模型在 Agent 路径上是降级而非崩坏。
